...
class Delaunator:
    ...
    def constructor(self, coords):
        ...
        self.hashSize = int(math.ceil(math.sqrt(n)))
    ...
    def update(self,coords):
        ...
        self.hullHash[int(self._hashKey(i0x, i0y))] = i0
        self.hullHash[int(self._hashKey(i1x, i1y))] = i1
        self.hullHash[int(self._hashKey(i2x, i2y))] = i2
        ...
        for k in range(0,len(self._ids)):
            ...
            for j in range(0,self.hashSize):
                start = self.hullHash[int((key + j) % self.hashSize)]
            ...
            self.hullHash[int(self._hashKey(x, y))] = i
            self.hullHash[int(self._hashKey(coords[2 * e], coords[2 * e + 1]))] = e
    ...
    def get_triangles(self):
        tri = []
        for q in range(0,len(self.triangles), 3):
            x = []
            x.append(self.p[self.triangles[q]])
            x.append(self.p[self.triangles[q+1]])
            x.append(self.p[self.triangles[q+2]])
            tri.append(x)
            # yield tri
        return tri
